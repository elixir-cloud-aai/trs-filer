...
MOCK_REQUEST_DATA_VALID_FALSE_VALIDATION = {
    "aliases": [
        "630d31c3-381e-488d-b639-ce5d047a0142",
        "dockstore.org:630d31c3-381e-488d-b639-ce5d047a0142",
        "bio.tools:630d31c3-381e-488d-b639-ce5d047a0142"
...
def test_toolClass__with_validation_false():
    """Test for user supplying ToolClass id with no_validation False."""
    app = Flask(__name__)
    app.config['FOCA'] = Config(
        db=MongoConfig(**MONGO_CONFIG),
        endpoints=ENDPOINT_CONFIG_CHARSET_EXPRESSION,
    ...
    app.config['FOCA'].db.dbs['trsStore'] \
        .collections['toolclasses'].client = MagicMock()
    mock = MagicMock(side_effect=[BadRequest(''), None])
    app.config['FOCA'].db.dbs['trsStore'] \
        .collections['toolclasses'].client.insert_one = mock

    request_data = Dict()
    request_data.json = MOCK_REQUEST_DATA_VALID_FALSE_VALIDATION
    with app.app_context():
        assert isinstance(
            RegisterObject(request_data).register_object()['id'],
            str,
